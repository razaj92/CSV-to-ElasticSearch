...
def main(file_path, delimiter, max_rows, elastic_index, json_struct, datetime_field, elastic_type, elastic_address, ssl, username, password, id_column):
    ...
    print("Importing %s rows into `%s` from '%s'" %
          (max_rows_disp, elastic_index, file_path))
    print("")
    ...
    with open(file_path, 'r') as csvfile:
        ...
        for row in reader:
            if count == 0:
                ...
                for header in headers:
                    if header == datetime_field:
                        ...
                        _data = _data.replace(
                            '"%' + header + '%"', '"{:%Y-%m-%dT%H:%M}"'.format(datetime_type))
                    else:
                        try:
                            int(row[pos])
                            _data = _data.replace(
            ...
            count += 1
            if count % 10000 == 0:
                send_to_elastic(elastic_address, endpoint, ssl,
                                username, password, to_elastic_string, count)
                to_elastic_string = ""

    print('Reached end of CSV - sending to Elastic')
    send_to_elastic(elastic_address, endpoint, ssl, username,
                    password, to_elastic_string, count)
...
def send_to_elastic(elastic_address, endpoint, ssl, username, password, to_elastic_string, block=0):
    ...
    if not username is None:
        auth = b64encode(bytes("{}:{}".format(
            username, password), "utf-8")).decode("ascii")
        ...
        if response_details['errors']:
            ...
            for i in response_details['items']:
                ...
                if i['index']['status'] != 201:
                    print("\n*** Problem on line {}: {}".format(block +
                                                                line, i['index']['error']))
            print("\n*** Error occured during import. See response body above. ***\n")
        else:
            print("Import of {} items was successful.".format(
                len(response_details['items'])))
        return
